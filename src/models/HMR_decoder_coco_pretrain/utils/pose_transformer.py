'''
Credits to https://github.com/shubham-goel/4D-Humans/blob/main/hmr2/models/components/pose_transformer.py
for this file
'''
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # these are the q k v matrices, and the map applies the function to each of them
            # notice that this is multihead, since there are b = batche, n = dimensinos of q/k/v, h = number of heads, d = number of dimension per head
        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), qkv)

        # multiply all the different q with all the different k
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args):
        for attn, ff in self.layers:
            # just the transformer architecture. attn followed by feedforward. sum both
            x = attn(x, *args) + x
            x = ff(x, *args) + x
        return x
